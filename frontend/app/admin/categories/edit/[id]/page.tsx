...
const EditCategoryPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    name: 'Heritage',
    slug: 'heritage',
    description: 'Classic vintage-inspired timepieces with timeless elegance',
    status: 'Active'
  });
  const [imagePreview, setImagePreview] = useState<string | null>('/categories/heritage.jpg');
  const [imageFile, setImageFile] = useState<File | null>(null);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Category updated:', { ...formData, image: imageFile });
      ...
      <div className="bg-white border border-gray-200 p-4 sm:p-6 mb-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-serif font-light text-black">Edit Category</h2>
              <p className="text-sm text-gray-400 font-sans font-light">Update category #{id}</p>
            </div>
          </div>
          <Link
